...
class Tank:
    ...
    def move(self, otherTank):
        ...
        if keys[pygame.K_UP]:
            ...
            #     self.x -= math.cos((otherTank.angle / 180) * math.pi) * self.velocity[0]
            #     self.y += math.sin((otherTank.angle / 180) * math.pi) * self.velocity[1]
                # otherTank.collision()
...
redTank = Tank("red", starting_data[0],starting_data[1])
redTank.angle = starting_data[2]
blueTank = Tank("blue", 250, 250)
...
while run:
    ...
    sending_data = (redTank.x, redTank.y, redTank.angle)
    receivedData = (read_data(n.send(make_tank(sending_data))))
    blueTank.x = receivedData[0]
    blueTank.y = receivedData[1]
    blueTank.angle = receivedData[2]
